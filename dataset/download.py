...
def download(localFile, retry_count=0, max_retries=5):
	...
	if retry_count >= max_retries:
		print(f"Failed to download {localFile} after {max_retries} retries. Skipping...")
		return
	
	print("------------------------------------------------------------")
	print('Downloading %s (Attempt %d/%d)' % (localFile, retry_count+1, max_retries))
	try:
		with requests.get(srcUrl, stream=True, timeout=30) as r:
			if r.status_code != 200:
				print(f"Error: HTTP {r.status_code} for {srcUrl}")
				print("Retrying in 10 seconds...")
				time.sleep(10)
				return download(localFile, retry_count+1, max_retries)
		...
		print(f"\nTimeout error for {localFile}. Retrying in 30 seconds...")
		time.sleep(30)
		return download(localFile, retry_count+1, max_retries)
	except requests.exceptions.ConnectionError:
		print(f"\nConnection error for {localFile}. Retrying in 60 seconds...")
		time.sleep(60)
		return download(localFile, retry_count+1, max_retries)
