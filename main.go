...
func main() {
	...
	// ExClique flag
	pflag.BoolVarP(&useExClique, "exclique", "e", false, "useExClique is a bool value, which indicates whether to use ExClique consensus instead of PBFT.")
	...
	if isSupervisor {
		...
		if useExClique {
			log.Printf("Starting node with ExClique consensus\n")
			build.BuildNewExCliqueNode(uint64(nodeID), uint64(nodeNum), uint64(shardID), uint64(shardNum))
		} else {
			log.Printf("Starting node with PBFT consensus\n")
