	...
	for {
		...
		case <-nextRoundBeginSignal:
			go func() {
				...
				networks.Broadcast(p.RunningNode.IPaddr, p.getNeighborNodes(), p.encodeProposal(ppmsg))
...
func (p *PbftConsensusNode) handlePrePrepare(content []byte) {
	...
	curView := p.view.Load()
	if params.Overhead.Enabled && ppmsg.RequestMsg.RequestType == message.PartitionReq {
		networks.MarkMigrationDigest(ppmsg.Digest)
	}
	p.pbftLock.Lock()
