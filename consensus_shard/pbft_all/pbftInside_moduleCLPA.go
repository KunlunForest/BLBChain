...
func (cphm *CLPAPbftInsideExtraHandleMod) HandleinPropose() (bool, *message.Request) {
	cphm.cdm.StateLock.Lock()
	partitionOn := cphm.cdm.PartitionOn
	cphm.cdm.StateLock.Unlock()
	if partitionOn {
		experiment.Event("migration_pause_start", cphm.pbftNode.ShardID, cphm.pbftNode.NodeID, cphm.cdm.AccountTransferRound+1, 0)
...
func (cphm *CLPAPbftInsideExtraHandleMod) HandleinCommit(cmsg *message.Commit) bool {
	...
	if cphm.pbftNode.NodeID == uint64(cphm.pbftNode.view.Load()) {
		...
		go func() {
			if err := networks.TcpDial(msg_send, cphm.pbftNode.ip_nodeTable[params.SupervisorShard][0]); err != nil {
